% Blackjack Problem using Monte Carlo - Epsilon Greedy
...
while iter < max_iter
    ...
    action = get_policy_epsilon_greedy(state, Q, epsilon); % epsilon-greedy
    ...
    while player_turn
        ...
        if player_turn
            action = get_policy_epsilon_greedy(state, Q, epsilon);
        end
    end
...
function action = get_policy_epsilon_greedy(state, Q, epsilon)
    e = rand(1);
    if e > epsilon
